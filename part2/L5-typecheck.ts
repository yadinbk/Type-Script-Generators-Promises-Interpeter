...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, valuesExp, LetValuesExp, VarDecl, ValuesBinding, CExp } from "./L5-ast";
...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    ...
    isValuesExp(exp) ? typeofValue(exp, tenv) :
...
/*
export const BindingsCheck = (vars: VarDecl[][], vals: CExp[]): Boolean =>{
    const vars_lenght = map ((x:VarDecl[]) => x.length, vars)
    const vals_lenght = map ((x:CExp) => isValuesExp(x)? x.values.length : -1, vals)
...
// Purpose: compute the type of a values-exp
export const typeofValue = (val: valuesExp, tenv: TEnv): Result<TExp> => {
    const values = mapResult((x: CExp) => typeofExp(x, tenv) , val.val)
    return isOk(values)? isEmpty(values.value)? makeOk(makeEmptyTupleTExp()):
        allT(isNonTupleTExp, values.value)? makeOk(makeNonEmptyTupleTExp(values.value)):
        makeFailure("Not a tuple") : values;
};

// Purpose: compute the type of a letValues-exp
export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> =>{
    const vars = map((binds:ValuesBinding) => binds.vars, exp.binding);
    const vals = map((binds:ValuesBinding) => binds.val, exp.binding);
    const valsToValue = (allT(isValuesExp, vals))? map((x: valuesExp) => x.val, vals).reduce ((acc, curr) => acc.concat(curr), []) : [];
    const varsToTEXP = vars.reduce((acc: VarDecl[], cur:VarDecl[]) => acc.concat(cur), []). map((x:VarDecl) => x.texp);
    const varsToVar = vars.reduce((acc: VarDecl[], cur:VarDecl[]) => acc.concat(cur), []). map((x:VarDecl) => x.var);
    const varsVals = zipWithResult((varTExp, val) => bind(typeofExp(val, tenv),
                                                           (valTExp: TExp) => checkEqualType(varTExp, valTExp, exp)),
                                                           varsToTEXP, valsToValue);
    return bind(varsVals, _ => typeofExps(exp.body, makeExtendTEnv(varsToVar, varsToTEXP, tenv)));
};
