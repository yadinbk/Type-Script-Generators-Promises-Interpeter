...
import { join, map, zipWith, values, empty } from "ramda";
...
//         |  ( <cexp> <cexp>*f )              / AppExp(operator:CExp, operands:CExp[]))
...
export interface LetValuesExp {tag: "LetValuesExp"; binding: ValuesBinding[]; body: CExp[]; }
export const makeLetValuesExp = ( bindings: ValuesBinding[] ,body: CExp[]): LetValuesExp =>
    ({tag: "LetValuesExp", binding:bindings , body:body});
...
export const makeValBinding = (vars: VarDecl[], val: CExp): ValuesBinding=>
({tag: "ValuesBinding", vars: vars,val: val});
export const isValBinding = (x:any): x is ValuesBinding => x.tag === "ValuesBinding";
...
const isSpecialForm = (x: string): boolean =>
    ["if", "lambda", "let", "quote", "letrec", "values", "set!", "let-values"].includes(x);
...
const parsevaluesExp = (params: Sexp[]): Result<valuesExp> =>
    ...
    safe2((first: CExp, rest: CExp[])=> makeOk(makevaluesExp(rest)))
    (parseL5CExp(first(params)), mapResult(parseL5CExp,params));
...
const parseLetValuesExp= (binding: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    isEmpty(body) ?makeOk(makeLetValuesExp([],[])) :
    ! isGoodValBindings(binding) ? makeFailure(`Invalid bindings: ${JSON.stringify(binding)}`) :
    safe2((bdgs: ValuesBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
        (parseValBinding(binding), mapResult(parseL5CExp, body));
...
const parseValBinding = (binding:[Sexp[], Sexp][]): Result<ValuesBinding[]> =>
    safe2((vds: VarDecl[][], vals: CExp[]) =>makeOk(zipWith(makeValBinding, vds, vals)))
    (mapResult(parseVarDeclArr, map(b => b[0], binding)), mapResult(parseL5CExp, map(b => b[1], binding)));

const parseVarDeclArr = (v: Sexp[]): Result<VarDecl[]> =>
    mapResult(parseVarDecl,v);
...
const unparseValBindings = (binding: ValuesBinding[]): Result<string> =>  
    bind(mapResult(bdg => safe2((vd: string, val: string) => makeOk(`(${vd} ${val})`))(unparseVarDeclArr(bdg.vars), unparse(bdg.val)), binding),
        (bdgs: string[]) => makeOk(join(" ", bdgs)));
...
const unparseVarDeclArr = (vda: VarDecl[]): Result<string> =>
        bind(mapResult(v => unparseVarDecl(v),vda), (bdgs: string[]) => makeOk(`(${join(" ", bdgs)})`));
...
const unparsevaluesExp = (vexp:valuesExp): Result<string> =>
        bind(mapResult(unparse,vexp.val),(v: string[])=>makeOk(`(values ${join(" ",v)})`));
...
(unparseValBindings(lve.binding), unparseLExps(lve.body));
