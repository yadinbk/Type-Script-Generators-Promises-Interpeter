...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (pos === -1)  ? bind (parseTupleTExp(texps), (x: TExp[]) => isEmpty(x)? makeOk(makeEmptyTupleTExp()): allT(isNonTupleTExp, x)? makeOk(makeNonEmptyTupleTExp(x)) : makeFailure(`Procedure type expression without -> - ${texps}`)) :
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isNonEmptyTupleTExp(x)? bind(unparseTuple(x.TEs), (y: string []) => makeOk(y)):
        makeFailure("Never");
